import { Controller, UseInterceptors, Get, Post, Body, Put, Request, Response, Param, Query, Delete, ParseIntPipe, UseGuards } from '@nestjs/common';
...
export class UserController {
  constructor(private readonly userService: UserService, private readonly userFriendCategoryService: UserFriendCategoryService) {}
  ...
  async getUserFriendCategoriesByUserId(
    @Param('id', new ParseIntPipe())
    id: number,
  ) {
  ...
  async getUserPagingByFilter(@Body() userFilterDto: UserFilterDto, @Query('pageNum') pageNum: number, @Query('pageSize') pageSize: number) {
