...
export class UserFriendCategoryService {
  ...
  public async createUserFriendCategory(
    createUserFriendCategoryDto: CreateUserFriendCategoryDto,
  ): Promise<number> {
  ...
  public async updateUserFriendCategory(
    userFriendCategoryId: number,
    updateUserFriendCategoryDto: UpdateUserFriendCategoryDto,
  ): Promise<void> {
    let conn: IConnection;
    let beginTrans: boolean = false;
    try {
      if (ObjectUtils.isNullOrUndefined(userFriendCategoryId)) {
  ...
  public async deleteUserFriendCategory(
    userFriendCategoryId: number,
  ): Promise<void> {
    let conn: IConnection;
    let beginTrans: boolean = false;
  ...
  public async getUserFriendCategoriesByUserId(
    userId: number,
  ): Promise<UserFriendCategoryDto[]> {
    let conn: IConnection;
    try {
      ...
      const userIdFilter = new FilterDescriptor<UserFriendCategory>(
        g => g.userId,
        FilterOperator.EQUAL,
        userId,
      );
      const indexSort = new SortDescriptor<UserFriendCategory>(
        g => g.categoryIndex,
      );
      const idSort = new SortDescriptor<UserFriendCategory>(
        g => g.id,
        SortDirection.DESC,
      );
      ...
      return new Promise<UserFriendCategoryDto[]>((resolve, reject) =>
        resolve(result),
      );
    } catch (error) {
      return new Promise<UserFriendCategoryDto[]>((resolve, reject) =>
  ...
  private createDtoToEntity(
    ...
    userFriendCategory.categoryIndex =
      createUserFriendCategoryDto.categoryIndex;
  ...
  private updateDtoToEntity(
    ...
    userFriendCategory.categoryIndex =
      updateUserFriendCategoryDto.categoryIndex;
    userFriendCategory.updateTime = new Date();
    return userFriendCategory;
  }
