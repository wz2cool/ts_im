...
export class AuthService {
    ...
    public async createToken(userLoginInfo: UserLoginInfoDto): Promise<JwtTokenDto> {
        try {
            const user = await this.userService.getUserBaseInfoByLoginInfo(userLoginInfo);
            ...
            await this.redisService.setWithExpire(this.generateUserIdLoginKey(user.id), JSON.stringify(user), expiresIn);
    ...
    public async validateUser(payload: JwtPayload): Promise<string> {
        try {
            const userStr = await this.redisService.get(this.generateUserIdLoginKey(payload.userId));
