...
export class UserService {
        ...
        try {
            ...
            if (CommonHelper.isNotBlank(userFilterDto.displayName)) {
                const displayNameFilter = new FilterDescriptor<User>(
                    x => x.displayName, FilterOperator.CONTAINS, userFilterDto.displayName);
    ...
    private entityToUserInfoDto(user: User): UserInfoDto {
        ...
        dto.createTime = DateHelper.getTimestamp(user.createTime);
        dto.updateTime = DateHelper.getTimestamp(user.updateTime);
        return dto;
    }
